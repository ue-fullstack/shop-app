...
export const searchShops = (page: number, size: number, query: string): Promise<ResponseArray<Shop>> => {
    return axios.get(`${process.env.REACT_APP_API}/search?page=${page}&size=${size}&query=${query}`);
}
